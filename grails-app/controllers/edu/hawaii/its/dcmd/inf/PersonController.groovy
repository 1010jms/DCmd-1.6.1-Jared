...
//                     println(Person.all.get(i).properties['uhNumber','uhName','title','lastName','firstName','midInit','telephone','primaryEmail'])
    ...
    def listAllPerson = {
        ...
        def person = Person.createCriteria().list(max: maxRows, offset: rowOffset) {
        ...
        def totalRows = person.totalCount
        def numberOfPages = Math.ceil(totalRows / maxRows)

        def results = person?.collect { [ cell: [
                "<a onclick='openItem(${it.id})'>${it.uhName}</a>",
                "<a onclick='openItem(${it.id})'>${it.lastName}</a>",
                "<a onclick='openItem(${it.id})'>${it.firstName}</a>",
                it.midInit,
                it.title,
                it.telephone,
                it.primaryEmail,
                "<a onclick='openItem(${it.id})'>${it.uhNumber}</a>",
                it.generalNote],
                id: it.id ] }
...
//        refresh = true;
    ...
    def listAll = {
        ...
        def jsonData = Person.get(params)
        render jsonData as JSON
    }
    ...
    def exportListAll = {
        ...
        def jsonData = personService.listAllPerson(params)
        def theList = []
        jsonData.rows.each { theRow ->
            ...
            def tempVals = [
                    'UH Username': theRow.cell[0].replaceAll("\\<.*?>",""),
                    'Last Name': theRow.cell[1],
                    'First Name': theRow.cell[2],
                    'MI':theRow.cell[3].replaceAll("\\<.*?>",""),
                    'Title': theRow.cell[4].replaceAll("\\<.*?>",""),
                    'Phone #': theRow.cell[5],
                    'E-mail': theRow.cell[6].replaceAll("\\<.*?>",""),
                    'UH ID': theRow.cell[7],
                    'General Notes': theRow.cell[8].replaceAll("\\<.*?>",""),
                    'id': theRow.cell[9],
            ]
...
//                        message = "Replacement ${params.replacement} for Asset ${params.main_asset} Deleted."
    ...
    def getPersonIdByServer = {
        def theRole = SupportRole.createCriteria().list {
            eq('id', params.serverId.toLong())
        }
        JSON.use("theSupportStaffGrid") {
            respond theRole
        }
    }

    def getPersonIdByHost = {
        def theRole = SupportRole.createCriteria().list {
            eq('id', params.hostId.toLong())
        }
        JSON.use("theSupportStaffGrid") {
            respond theRole
        }
    ...
    def getPersonIdByApp = {
        def theRole = SupportRole.createCriteria().list {
            eq('id', params.appId.toLong())
        }
        JSON.use("theSupportStaffGrid") {
            respond theRole
        }
    ...
    def getPersonDetails = {
        ...
        def person = Person.get(params.staffId)
        //render person as JSON
        def response = [retVal: true,
                        uhName:person.uhName,
                        id:person.id,
                        lastName:person.lastName,
                        firstName:person.firstName,
                        midInit:person.midInit,
                        title:person.title,
                        uhNumber:person.uhNumber,

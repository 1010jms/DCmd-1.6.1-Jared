...
//        resetCloneVar()
    ...
    * List action contains code for spreadsheet file export
    ...
    def listAllHost = {
        ...
        def jsonData = hostService.listAllHost(params)
...
*/
    ...
    def getHostsByService = {
        ...
        def hosts = Host.createCriteria().list() {
            eq('asset.id', params.serviceId.toLong())
        }
        JSON.use("serverHostGrid") {
            respond hosts
        }
    ...
    def getHostsByCluster = {
        ...
        def hosts = Host.createCriteria().list() {
            eq('asset.id', params.clusterId.toLong())
        }
        JSON.use("serverHostGrid") {
            respond hosts
        }
    ...
    def getTierDetails = {
        System.out.println(params)
        def tiers = Tier.get(params.id)
        def response = [retVal: true,
                        application:tiers.mainApp?.applicationTitle,
                        service:tiers.services?.service,
                        servAdmin:tiers.services,
                        loadBalanced:tiers.loadBalanced,
                        tierType: tiers.tierName,
                        generalNote:tiers.generalNote,
                        id:tiers.id]
        render response as JSON
    ...
    def getServersAsSelect = {
        def theList = []
        PhysicalServer.getAll().each { server ->
            theList.add(['id':server.id, 'text':server.itsId])
        }
        render theList as JSON
    }
}

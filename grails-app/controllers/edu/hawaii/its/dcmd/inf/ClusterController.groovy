...
// Sort
        ...
        def results = clusters?.collect { [ cell: [
                "<a onclick='openItem(${it.id})'>${it.name}</a>", //it.name,
                it.assetSet.size(),
                it.hostSet.size(),
                it.lastUpdated.format('MM/dd/yy h:m a'),
                it.generalNote],
                id: it.id ] }
    ...
    def exportListAll = {
        ...
        jsonData.rows.each { theRow ->
            ...
            def tempVals = [
                    'Cluster Name': theRow.cell[0].replaceAll("\\<.*?>",""),
                    '# of Servers': theRow.cell[1],
                    '# of Hosts': theRow.cell[2],
                    'Last Updated':theRow.cell[3].replaceAll("\\<.*?>",""),
                    'Notes': theRow.cell[4].replaceAll("\\<.*?>",""),
            ]
...
//                        message = "Replacement ${params.replacement} for Asset ${params.main_asset} Deleted."
    ...
    def getClusterDetails = {
        System.out.println(params)
        def cluster = Cluster.get(params.clusterId)

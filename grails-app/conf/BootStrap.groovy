    ...
    def init = { servletContext ->
        ...
        environments {
             ...
             production {
                 ...
                 JSON.createNamedConfig("hostServiceGrid") {
                     it.registerObjectMarshaller(Tier) { Tier tier, JSON json ->
                         def returnArray = [:]
                         returnArray['Application'] = tier.mainApp?.applicationTitle
                         returnArray['Service'] = tier.services?.service
                         returnArray['Service Primary SA'] = tier.services
                         returnArray['Load Balanced'] = tier.loadBalanced
                         returnArray['Instance Type'] = tier.tierName
                         returnArray['Instance Notes'] = tier.generalNote
                         returnArray['id'] = tier.id

                         return returnArray
                     }
                 }

                 JSON.createNamedConfig("theSupportStaffGrid") {
                     it.registerObjectMarshaller(SupportRole) { SupportRole role, Person person, JSON json ->
                         def returnArray = [:]
                         returnArray['Role'] = role.roleName
                         returnArray['Person'] = role.person
                         returnArray['E-mail'] = person.primaryEmail
                         returnArray['Phone'] = person.telephone
                         returnArray['Notes'] = role.supportRoleNotes
                         returnArray['id'] = role.id
            ...
            test {
                ...
                JSON.createNamedConfig("hostServiceGrid") {
                    it.registerObjectMarshaller(Tier) { Tier tier, JSON json ->
                        ...
                        returnArray['application'] = tier.mainApp?.applicationTitle
                        returnArray['service'] = tier.services?.service
                        returnArray['servAdmin'] = tier.services
                        returnArray['loadBalanced'] = tier.loadBalanced
                        returnArray['tierType'] = tier.tierName
                        returnArray['generalNote'] = tier.generalNote
                        returnArray['id'] = tier.id

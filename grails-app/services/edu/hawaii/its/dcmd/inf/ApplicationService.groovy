...
// Sort
        ...
        def results = applications?.collect { [ cell:
        ["<a onclick='openItem(${it.id})'>${it.applicationTitle}</a>",
         it.env.toString(),
         it.applicationDescription,
         it.status.toString(),
...
// Sort
        ...
        def results = apps?.collect { [ cell: ['',
                                               it.applicationTitle,//"<a onclick='openSupportItem(${it.id})'>${it.applicationTitle}</a>",
                                               it.env.toString(),
...
//                resultTransformer Criteria.DISTINCT_ROOT_ENTITY
    ...
    try {
        // Secondary query on ids from previous (with filtering and sorting)...
        apps = Host.createCriteria().list(max: maxRows, offset: rowOffset) {
            'in'('id', appIds)
            // Search Filters
        ...
        totalRows = apps.totalCount
        numberOfPages = Math.ceil(totalRows / maxRows)

        results = apps?.collect { [ cell: [
                "<a onclick='openItem(${it.id})'>${it.applicationTitle}</a>",
                it.env.toString(),
                it.applicationDescription,
                it.status.toString(),
